...
- Replaced flag-based administration with `add`, `edit`, `delete`, `list`, and
  `config-path` subcommands. Removed `-a`, `-c`, `-d`, and `-e` shortcuts;
  `--print-config-path` is now `config-path`.

## [0.4.1] - 2026-09-02

### Added

- `--list --json` now prints the configured tools as compact JSON with
  `name`, `repository`, `version_command`, and `update_command` fields;
  `--json` without `--list` is rejected.

### Fixed

- `--add`, `--edit`, and `--delete` confirmations now print a leading
...
- Normal update runs now replace a diagnostic `delta.log` beside the selected
  configuration, including command, version, and failure details.
...
- GitHub release tags ending in stable versions, including Bun's `bun-vX.Y.Z` format, now compare correctly.
...
[0.5.0]: https://github.com/wsouto/delta/compare/v0.4.1...v0.5.0
[0.4.1]: https://github.com/wsouto/delta/compare/v0.4.0...v0.4.1
[0.4.0]: https://github.com/wsouto/delta/compare/v0.3.1...v0.4.0
[0.3.1]: https://github.com/wsouto/delta/compare/v0.3.0...v0.3.1
